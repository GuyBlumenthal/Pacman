...
class GameCell:
    ...
    def render(self, surface):
        if self.cell_type == CellType.WALL:
            ...
            pygame.draw.circle(surface, [255] * 3, (self.x * SIZE + SIZE / 2, self.y * SIZE + SIZE / 2), GameConstants.FOOD_RADIUS)
            ...
            pygame.draw.circle(surface, [255, 255, 191], (self.x * SIZE + SIZE / 2, self.y * SIZE + SIZE / 2), GameConstants.POWER_FOOD_RADIUS)
    ...
    def is_touching_food(self, center_loc, radius):
        ...
        if self.cell_type == CellType.FOOD:
            return distance <= GameConstants.FOOD_RADIUS
        if self.cell_type == CellType.POWER_FOOD:
            return distance <= GameConstants.POWER_FOOD_RADIUS
        else:
            return False
